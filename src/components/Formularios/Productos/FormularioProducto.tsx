...
const FormularioProductoInterno: React.FC = () => {
  ...
  const { data: registrosSanitarios = [], error: registrosSanitariosQueryError } = useQuery('registrosSanitarios', fetchRegistrosSanitarios, {
    onError: (error: any) => setRegistrosSanitariosError(`Error al cargar los registros sanitarios: ${error.message}`),
  });
    ...
    const lowercaseValue = (value: any): any => {
      if (typeof value === 'string') {
        ...
        return Object.fromEntries(
            Object.entries(value).map(([k, v]) => [k, lowercaseValue(v)])
        );
      }
      return value;
    };

    const mappedProducto = Object.entries(producto).reduce((acc, [key, value]) => {
  ...
  return (
      <div className="min-h-screen bg-[#eeeeee] text-[#333333] p-8">
        <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-md p-6">
          ...
          {registrosSanitariosError && <ErrorMessage message={registrosSanitariosError} onDismiss={() => setRegistrosSanitariosError(null)} />}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label htmlFor="technicalSheet" className="block text-sm font-medium text-[#00632C] mb-1">Ficha Técnica</label>
                <input
                    type="file"
                    id="technicalSheet"
